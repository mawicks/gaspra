...
def vserialize_int(value: int):
    ...
    if value <= 2287:
        excess = value - 240
        return int(241 + excess / 256).to_bytes(1) + int(excess % 256).to_bytes(1)
    if value <= 67823:
        excess = value - 2288
        return (
            int(249).to_bytes(1)
            + int(excess / 256).to_bytes(1)
            + int(excess % 256).to_bytes(1)
        )
    if value <= 16777215:
        code, length = 250, 3
        return int(250).to_bytes(1) + value.to_bytes(3)
    elif value <= 4294967295:
        code, length = 251, 4
        return int(251).to_bytes(1) + value.to_bytes(4)
    elif value <= 1099511627775:
        code, length = 252, 5
        return int(252).to_bytes(1) + value.to_bytes(5)
    elif value <= 281474976710655:
        code, length = 253, 6
        return int(253).to_bytes(1) + value.to_bytes(6)
    elif value <= 72057594037927935:
        code, length = 254, 7
        return int(254).to_bytes(1) + value.to_bytes(7)
    else:
        code, length = 255, 8
    return int(code).to_bytes(1) + value.to_bytes(length)
...
def vdeserialize_int(stream: bytes):
    ...
    a0 = int.from_bytes(stream[:1])
    if 0 <= a0 <= 240:
        return a0, stream[1:]
    if 241 <= a0 <= 248:
        a1 = int.from_bytes(stream[1:2])
        if len(stream) < 2:
            raise RuntimeError("Premature end of stream")
    ...
    if a0 == 249:
        if len(stream) < 3:
            raise RuntimeError("Premature end of stream")
        return 2288 + int.from_bytes(stream[1:3]), stream[3:]
    end = a0 - 246
    if len(stream) < end:
        raise RuntimeError("Premature end of stream")
    return int.from_bytes(stream[1:end]), stream[end:]
